...
The `product-audit` skill serves as the automated quality gate for product specifications. It calculates the **Spec Completeness Index (SCI)**, verifies cross-diagram consistency, validates schema pairing, and emits `AUDIT-PRD.md`.
...
- $N_{\text{total\_dimensions}} = 20$ (baseline dimension units)
...
4. **Registry Parity**: Number of inline `[UNKNOWN]` markers must match the count in Section 8 registry.
5. **Zero Fabrication**: All technical constraints must be grounded in raw notes or logged in Section 9 Decision Log.
...
1. Read `PRD.md`.
2. Execute the 5 verification gates.
